 ...
 #undef toCode
   ...
   vector<str> load(str filename)
   {
       ...
       while(!f.eof())
       {
          ...
          f>> tmp;
           str t(tmp);
            if (t==LSTR && res[res.size()-1]!="33")  //УЖАстнейший костыль :(((
            {
   ...
   // #define DEBUG_VM
void execute(vector<str> bytecode,unsigned long long int start)
{
    //виртуальная машина
  ...
  /*  for(int i = 0 ;i<bytecode.size();i++)
    ...
    for(unsigned long long int i=start;i<bytecode.size();)
    {
        ...
        if(bytecode[i]==NOT){
         #ifdef DEBUG_VM
                     log<<"noted:"<<stack[stack.size()-2]<<" "<<stack[stack.size()-1]<<" I:"<<i<<endl;
         #endif
          stack[stack.size()-1]=!stack[stack.size()-1];
